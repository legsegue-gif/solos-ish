...
struct fakefs_bind_mount {
    char path[PATH_MAX];      /* Linux path prefix, e.g. "/var/solos/offloads" */
 ...
 * e.g. "/Users/.../SolosChat/solos/<sid>/attachments/file.mp4"
 *   -> "/var/solos/attachments/file.mp4"
 ...
 * Handles both "/var/solos/..." and "var/solos/..." (without leading /). */
...
retry:
    ...
    if (strstr(entry_path, "solos") != NULL || strstr(entry_path, "Library/SolosChat") != NULL)
...
void fakefs_install_change_consumer(fakefs_change_handler_t handler) {
    ...
    g_change_consumer_queue = dispatch_queue_create(
        "com.solos.ish.fakechange", DISPATCH_QUEUE_SERIAL);
